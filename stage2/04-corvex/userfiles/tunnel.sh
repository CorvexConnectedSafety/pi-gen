...
then
    ...
    sport=`printf '12%03d' "$subscriber"`
    echo "Remote tunnel on port $dport"
    autossh -f -N -p $port -o "StrictHostKeyChecking no" -i /home/corvex/tunnel_key -L 10037:localhost:37 -L 5667:$monitoring:5667 -L 3128:127.0.0.1:3128 -L 13306:127.0.0.1:3306 -L 8080:127.0.0.1:$sport $tunnelUser@$tunnelServer 2>&1
